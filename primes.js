...
// use an external array to memoize the getPrimes function
...
function getPrimes(n) {
  ...
  let nums = [];
  // add these in reverse so we can pop them later (more efficient than shift)
  for (let i = limit; i > primes[primes.length - 1]; i--) {
    nums.push(i);   
  }
  // eliminate from nums all multiples of primes already found (if any)
  for (let i = 0; i < primes.length; i++) {
    nums = nums.filter(j => j % primes[i] !== 0);    
  }
  // now we start finding the additional primes we need 
  while (primes.length < n) {
    // grab the last (smallest) number in nums (necessarily a prime!)
    ...
    // now the new last (smallest) num is guaranteed to be prime again!
  }
  return primes;
}
